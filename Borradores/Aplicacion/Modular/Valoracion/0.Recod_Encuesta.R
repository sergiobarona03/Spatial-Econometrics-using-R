...
encuesta <- readxl::read_excel("Datos.xls")
...
# Convertir la valoración monetaria a dolares
trm = 3990
encuesta$TCi_usd = encuesta$TCi/trm

# Calcular el promedio simple (valoración monetaria)
encuesta$mean_es = NA
for (k in 1:nrow(encuesta)) {
  encuesta$mean_es[k] <- mean(c(encuesta$es18[k],
                                         encuesta$es19[k],
                                         encuesta$es20[k],
                                         encuesta$es21[k],
                                         encuesta$es22[k],
                                         encuesta$es23[k]), na.rm = TRUE)
  
}

# Guardar la encuesta recodificada
